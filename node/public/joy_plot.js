async function main() {
  const coverage = await d3.tsv(
    "data/multisample_plot_data_2.tsv",
    d3.autoType
  );
  ...
  // Create scales object to pass to external functions
  const scales = { x, z };

  // Event handlers now call external functions with necessary parameters
  group.on("click", (event, d) =>
    showSingleCoveragePlot(event, d, coverage, tooltip)
  );
  ...
  group
    ...
    .on("mousemove", (event, d) =>
      showToolTip(event, d, scales, position, tooltip)
    )
...
function showToolTip(event, d, scales, position, tooltip) {
  const { x, z } = scales;
  ...
  // Marker dot
  let marker = d3.select(event.currentTarget).select(".hover-dot");
...
function showSingleCoveragePlot(event, d, coverage, tooltip) {
  ...
  // Append graph
  const container = modalContent.append("div").attr("id", "single_plot").node();
